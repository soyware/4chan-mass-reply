...
function drawWatermarkReddit(file, subreddit, username, quality = 0.9) {
  return new Promise((resolve) => {
    const reader = new FileReader();
    reader.addEventListener('load', () => {
      const img = new Image();
      img.onload = () => {
        ...
        document.fonts.ready.then(() => {
          ...
          const by = ' by u/';
          const finalText = `${postedIn}${subreddit}${by}${username}`;

          debugLog(`Reddit watermark: ${finalText}`);

          // make sure text doesn't overlap logo
          const textW = ctx.measureText(finalText).width;
...
export function drawWatermark(file, type, subreddit, username, quality) {
  if (file.type !== 'image/jpeg' && file.type !== 'image/png') {
    return Promise.resolve(file);
  }

  if (type === 'reddit') return drawWatermarkReddit(file, subreddit, username, quality);
